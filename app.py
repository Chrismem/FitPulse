...
st.markdown(
    ...
    /* --- GYM SESSIONS: muscle picker grid ---
       All 15 muscle-group buttons live inside ONE container keyed
       "muscle_picker_grid", so this single rule (targeting every
       button inside it) styles all of them the same way — no need
       for a separate CSS block per muscle. Same card look as the
       home page feature cards, just a bit more compact. */
    .st-key-muscle_picker_grid button {
        background-color: #FFFFFF !important;
        border: 1px solid #E0E0E0 !important;
        border-top: 4px solid #7C9473 !important;
        border-radius: 14px !important;
    ...
    .st-key-muscle_picker_grid button:hover {
        transform: translateY(-4px);
        box-shadow: 0 10px 22px rgba(0,0,0,0.14) !important;
        border-top-color: #2A2A26 !important;
    }
    .st-key-muscle_picker_grid button:active {
        transform: translateY(-1px);
    }
    .st-key-muscle_picker_grid button p {
        margin: 4px 0 0 0;
        white-space: pre-line;
    }
    .st-key-muscle_picker_grid button p:first-of-type {
        font-size: 26px;
        margin-top: 0;
    }
    .st-key-muscle_picker_grid button p strong {
        color: #2A2A26;
        font-size: 16px;
    }
...
# ------------------------------------------------------------------
# ------------------------------------------------------------------
# GYM SESSIONS DATA: one entry per muscle group. MUSCLE_GROUPS drives
# the picker screen's buttons; MUSCLE_WORKOUT_CONTENT holds each
# muscle's full workout plan (looked up by key when a card is clicked).
# ------------------------------------------------------------------
MUSCLE_GROUPS = [
    ("chest", "💪", "Chest"),
    ("back", "🔙", "Back"),
    ...
    ("calves", "🦶", "Calves"),
    ("adductors", "🤎", "Adductors"),
    ("abductors", "➡️", "Abductors"),
]

MUSCLE_WORKOUT_CONTENT = {
...
"""),
    "shoulders": ("## 🔱 SHOULDERS WORKOUT PLAN", """
**Primary Exercise:**
...
- Barbell Skull Crushers: 3 sets x 12-15 reps
  ...
  - Progression: Increase weight or reps
"""),
    "forearms": ("## ✋ FOREARMS WORKOUT PLAN", """
**Primary Exercise:**
...
- Weighted Woodchoppers: 3 sets x 10-12 reps (each side)
  ...
  - Progression: Increase weight or reps
"""),
    "obliques": ("## ↪️ OBLIQUES WORKOUT PLAN", """
**Primary Exercise:**
...
"""),
    "lower_back": ("## 🏔️ LOWER BACK/ERECTORS WORKOUT PLAN", """
**Primary Exercise:**
...
- Bulgarian Split Squats: 3 sets x 10-12 reps (each leg)
  ...
  - Progression: Increase weight or reps
"""),
    "hamstrings": ("## 👙 HAMSTRINGS WORKOUT PLAN", """
**Primary Exercise:**
...
"""),
    "adductors": ("## 🤎 INNER THIGHS/ADDUCTORS WORKOUT PLAN", """
**Primary Exercise:**
- Machine Leg Press (Wide Stance): 3 sets x 10-12 reps
  - Rest: 90-120 seconds
...
- Goblet Squats (Wide Stance): 3 sets x 15-20 reps
  ...
  - Progression: Increase dumbbell weight or reps
"""),
    "abductors": ("## ➡️ OUTER THIGHS/ABDUCTORS WORKOUT PLAN", """
**Primary Exercise:**
- Machine Hip Abduction: 3 sets x 12-15 reps
  - Rest: 60-90 seconds
...
def render_gym_sessions():
    """
    Gym Sessions works as a two-step flow, both inside this SAME page
    (no extra top-level button was added anywhere — this is still the
    one "Gym Sessions" card at the bottom of the home page):

      Step 1 ("picker"): a fun "What do you want to work out today?"
      screen with a clickable card for every muscle group.

      Step 2 ("detail"): clicking a muscle card shows ONLY that
      muscle's full workout plan (reps, machines, form, progression),
      with a way to go back and pick a different muscle.

    st.session_state.gym_session_view remembers which of the two
    steps we're on, and st.session_state.selected_muscle remembers
    which muscle was picked.
    """
    if "gym_session_view" not in st.session_state:
        st.session_state.gym_session_view = "picker"
    if "selected_muscle" not in st.session_state:
        st.session_state.selected_muscle = None

    # ---------------- STEP 2: single muscle detail page ----------------
    if st.session_state.gym_session_view == "detail" and st.session_state.selected_muscle:
        muscle_key = st.session_state.selected_muscle
        header, body = MUSCLE_WORKOUT_CONTENT[muscle_key]

        nav_cols = st.columns([1, 1, 3])
        with nav_cols[0]:
            if st.button("⬅️ Choose Another Muscle", key="back_to_muscle_picker"):
                st.session_state.gym_session_view = "picker"
                st.rerun()
        with nav_cols[1]:
            if st.button("🏠 Back to Home", key="back_home_gym_detail"):
                st.session_state.page = "home"
                st.session_state.gym_session_view = "picker"
                st.rerun()
    ...
    # ---------------- STEP 1: fun muscle picker screen ----------------
    st.markdown(
        '''
        <div class="fitpulse-hero">
            <div class="fitpulse-hero-badge">Gym Sessions</div>
            <h1>🔥 What do you want to work out today?</h1>
            <p>Pick a muscle group below to pull up its full workout plan —
            reps, machines, form cues, and progression tips included.</p>
        </div>
        ''',
        unsafe_allow_html=True,
    )
    ...
    # All 15 muscle cards share ONE container key, so a single CSS
    # rule (see SECTION 2) styles every button inside it identically —
    # no need for a separate CSS block per muscle.
    with st.container(key="muscle_picker_grid"):
        cols = st.columns(3)
        for i, (muscle_key, emoji, label) in enumerate(MUSCLE_GROUPS):
            with cols[i % 3]:
                clicked = st.button(
                    f"{emoji}\n\n**{label}**",
                    key=f"muscle_card_{muscle_key}",
                    use_container_width=True,
                )
                if clicked:
                    st.session_state.selected_muscle = muscle_key
                    st.session_state.gym_session_view = "detail"
                    st.rerun()
